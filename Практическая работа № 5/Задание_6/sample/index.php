...
<body>
    ...
    foreach ($albums as $key => $item) {
        $display_key = $key < 9 ? "00" . ($key + 1) : "0" . ($key + 1);
        $clean_label  = rtrim(strip_tags(str_replace('</li>', ', ', $item['label'])), ', ');
        $clean_format = rtrim(strip_tags(str_replace('</li>', ', ', $item['format'])), ', ');
        $clean_status = rtrim(strip_tags(str_replace('</li>', ', ', $item['status'])), ', ');

        $out .= "
            Номер — {$display_key}<br />
            ID альбома: [{$item['id']}]<br />
            Название: {$item['album_name']}<br />
            Дата выпуска: {$item['date']}<br />
            Лейбл: {$clean_label}<br />
            Формат: {$clean_format}<br />
            Статус: {$clean_status}<p>
            <hr />
        ";
    }
