...
<head>
    <meta charset="UTF-8">
    <title>Задание №7 — Многомерный массив</title>
</head>
<body>

    <h1>Многомерная дискография Pink Floyd (Атомарные значения)</h1>
    ...
    $discography = [
        ...
        ["id" => "9", "name" => "A Momentary Lapse of Reason", "date" => "8 сентября 1987", "label" => "EMI, Columbia", "format" => "LP, Кассета, CD", "status" => "Платиновый (USA), Золотой (GBR), Платиновый (CAN), Золотой(NLD)"],
    ...
    foreach ($discography as $key => $album) {
        $discography[$key]['label']  = explode(", ", $album['label']);
        $discography[$key]['format'] = explode(", ", $album['format']);
        $discography[$key]['status'] = explode(", ", $album['status']);
    }
